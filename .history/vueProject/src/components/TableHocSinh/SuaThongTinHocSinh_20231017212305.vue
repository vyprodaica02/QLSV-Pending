  ...
  <div v-if="isOpen" class="fixed inset-0 flex items-center justify-center">
    ...
    >
      ...
      <div class="mb-4">
        ...
        <div class="mb-4">
          <label for="NgaySinh" class="block text-gray-700">Ngày Sinh</label>
          <div class="relative" @click="showDatePicker = !showDatePicker">
            <img
              src="path_to_your_calendar_image"
              alt="calendar"
              class="w-6 h-6 cursor-pointer"
            />
            <div v-if="showDatePicker" class="absolute top-0 left-0 mt-8">
              <input
...
export default defineComponent({
  ...
  methods: {
    ...
    formatDate(dateTimeString) {
      if (!dateTimeString) return ""; // Kiểm tra xem chuỗi có tồn tại không
      const date = new Date(dateTimeString); // Tạo đối tượng ngày từ chuỗi
      const year = date.getFullYear();
      let month = (1 + date.getMonth()).toString();
      month = month.length > 1 ? month : "0" + month; // Đảm bảo định dạng tháng là 2 chữ số
      let day = date.getDate().toString();
      day = day.length > 1 ? day : "0" + day; // Đảm bảo định dạng ngày là 2 chữ số
