...
//
const getPermissionFromToken = (jwtToken) => {
  try {
    const decodedToken = jwt.verify(jwtToken, "your_secret_key_here");
    return decodedToken.permission; // Thay 'permission' bằng trường chứa quyền trong token của bạn
  } catch (error) {
    console.error("Invalid token or secret key");
    return null;
  }
};
// pagination
...
const deleteItem = (item, jwtToken) => {
  const userPermission = getPermissionFromToken(jwtToken);
